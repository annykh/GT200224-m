...
from countries t1 -- временное имя таблицы 
join regions t2 -- временное имя таблицы 
...
select t1.region_name, t2.country_name, t3.city, t3.street_address
from regions t1
join countries t2
on t1.region_id = t2.region_id
join locations t3
...
--  locations.city
select t1.first_name, t1.last_name, t1.salary, t3.city
from employees t1
join departments t2
on t1.department_id = t2.department_id
join locations t3
on t2.location_id = t3.location_id
where t3.city Like '%Oxford%' or t3.city Like '%San Francisco%';
