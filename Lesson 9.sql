...
select t1.city, t2.department_id
from locations t1
left join departments t2
on t1.location_id = t2.location_id
...
select t1.country_name, t2.location_id, t2.street_address, t2.postal_code, t2.city, t2.country_id
from countries t1
...
select emp.first_name, emp.last_name, mng.first_name, mng.last_name
from employees emp
join employees mng
on emp.manager_id = mng.employee_id
...
select emp.first_name as emp_first_name, emp.last_name as emp_last_name, mng.first_name as mng_first_name, 
mng.last_name as mng_last_name, emp.salary as emp_salary, mng.salary as mng_salary
from employees emp
join employees mng
on emp.manager_id = mng.employee_id
where emp.salary < mng.salary;

select emp.first_name as emp_first_name, emp.last_name as emp_last_name
from employees emp
join employees mng
on emp.manager_id = mng.employee_id
where emp.salary < mng.salary;

-- Вывести тех сотрудников, которые зарабатывают больше своих менеджеров.
select emp.first_name as emp_first_name, emp.last_name as emp_last_name
from employees emp
join employees mng
on emp.manager_id = mng.employee_id
...
-- 2. Вывести имя, фамилию и зарплару сотрудников
...
-- 1. Найти все города, где нет департаментов
select t1.city, t2.department_id
from locations t1
left join departments t2
on t1.location_id = t2.location_id
...
-- Вывести список сотрудников с должностью 'Programmer', чья зарплата выше средней зарплаты среди всех сотрудников с этой должностью.

-- 1. Найти среднюю зарплату среди всех сотрудников с должностью Programmer.
select avg(salary)
from employees t1
join jobs t2
on t1.job_id = t2.job_id
...
join jobs t2
on t1.job_id = t2.job_id
where t2.job_title = 'Programmer' and t1.salary > (select avg(salary)
													from employees t1
													join jobs t2
													on t1.job_id = t2.job_id
													where job_title = 'Programmer');
